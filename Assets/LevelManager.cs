...
public class LevelManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Debug.Log(equationCanvas.transform.forward.y + " " + CaveInABox.transform.forward.y);
        {
            ...
            lookAtTester.transform.SetPositionAndRotation(equationCanvas.transform.position, equationCanvas.transform.rotation);
            ...
            if (Vector3.Angle(equationCanvas.transform.forward, lookAtTester.transform.forward) < 150) //doesn't work yet, supposed to only turn if it's off by more than 1 degree
            {
            ...
            lookAtTester.transform.SetPositionAndRotation(settingsCanvas.transform.position, settingsCanvas.transform.rotation);
            lookAtTester.transform.LookAt(CaveInABox.transform);
            //Debug.Log(settingsCanvas.transform.rotation.y - CaveInABox.transform.rotation.y);
            if (Vector3.Angle(settingsCanvas.transform.forward, lookAtTester.transform.forward) < 150)
            {
    ...
    public void ResetBalls()
    {
        for (int i = 0; i < numberSpheres; i++)
        {
            ...
            SphereList[i].GetComponentInChildren<Animator>().SetBool("Start", false);
        }
        timerOn = false;
        ...
        {
            SphereList[j].transform.LookAt(CaveInABox.transform);
        }
    }


    public void LoadLevel(int levelNumber) //level data, can convert to xml
    {
        timerOn = false;
        switch (levelNumber)
        {
    ...
    public void StartLevel()
    {
        ...
        for (int i = 0; i < numberSpheres; i++)
        {
            ...
            if (SphereList[i].GetComponentInChildren<Animator>()!= null)
                SphereList[i].GetComponentInChildren<Animator>().SetBool("Start", true);
        }
    }
